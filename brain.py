...
class Brain:
    def __init__(self, inputs):
        ...
        # Create connections
        for i in range(0, 4):
            # Weights are a random number -1 to 1
    ...
    def generate_net(self):
        ...
        # Fresh net list
        self.net = []
        # Fills the net list with our nodes in order of layer
        for j in range(0, self.layers):
            for i in range(0, len(self.nodes)):
                if self.nodes[i].layer == j:
                    self.net.append(self.nodes[i])
    
    # Sets the output value of each input node
